...
{
  // pmk: disabled: uses MediaListener, which extends android::SurfaceTexture
#if 0
    android::Mutex::Autolock lock(m_mediaLock);
                                        ...
                                        FramerateCallbackProc callback)
{
    ...
    for (unsigned int i = 0; i < m_videoTextures.size(); i++) {
        if (m_videoTextures[i]->nativeWindow.get() == window) {
          // pmk: disabled: uses disabled class MediaTexture
...
{
  // pmk: disabled: uses android::SurfaceTextureClient
#if 0
...
{
  // pmk: disabled: uses android::SurfaceTexture
    ...
    // pmk: disabled: uses field with incomplete type: mediaListener
    // texture->mediaListener.clear();
    texture->nativeWindow.clear();
    // pmk: disabled: uses field with incomplete type: surfaceTexture
