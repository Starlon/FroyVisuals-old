...
public: 
    ...
    LuaFuncRef( const std::string& id, lua_State* lua, int stackPos )
        : LuaObjRef(id, lua, stackPos) {}
    //@}
...
public:
    ...
    /// create from LuaTempResult; res[0] is function, is object res[1]
    LuaBoundMethRef( const LuaTempResult & res )
        : mObjRef( res[0] ), mMethRef( res[1] ) {}

    //@{
    /// Change what the reference is pointing to
    void resetRef( const LuaObjRef & obj, const LuaFuncRef & fn );
    void resetRef( const LuaTempResult& result ) { resetRef(result[0], result[1]); }
...
public:
    ...
    LuaClassObjRef( const std::string& id, lua_State* lua, int stackPos )
        : LuaObjRef(id, lua, stackPos) {}
    //@}
