...
public:
    ...
    LuaTableRef( const std::string& id, lua_State* lua, int stackPos )
...
private:
    ...
    void pushFieldValue( int key ) const { lua_rawgeti(mLua, -1, key); }
    void pushFieldValue( const char* key ) const { lua_getfield(mLua, -1, key); }
...
{ 
    ...
    lua_remove(mTable.getLua(), -2);     // remove table (just need value)
}
