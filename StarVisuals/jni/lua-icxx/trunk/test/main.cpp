...
{
    cout << "Expected ERROR: " << res.getErrMsg() << endl;
    assert( ! res.ok() );
}
...
void testTempResult()
{
    ...
    // evaluation results
    {
        ...
        assert( res.ok() );
        assert( res.getNumItems() == 2 );

        LuaObjRef item1 = res[1];
        assert( item1 == 9 );
        assert( item1.getAs<int>()  == int(item1) );
        assert( item1.getTypeID()   == LUA_TNUMBER );
        assert( res[-2].getTypeID() == LUA_TNUMBER );

        LuaObjRef item2 = res[2];
        assert( item2.getTypeID() == LUA_TSTRING );
        assert( item2 == "ab" );
        assert( res[-1].getTypeID() == LUA_TSTRING );

        // indices that are invalid behave as nil: 
        assert( res[0].getTypeID()  == LUA_TNIL );
        assert( res[-3].getTypeID() == LUA_TNIL );
        assert( res[3].getTypeID()  == LUA_TNIL );
        assert( res[0].isNil()  );
        assert( res[-3].isNil() );
        assert( res[3].isNil()  );
    ...
    // multiple results
    {
        ...
        assert( res1.getNumItems() == 1 );
        assert( res2.getNumItems() == 2 );
        assert( res3.getNumItems() == 3 );

        assert( res1[1] == 3 );
        assert( res2[1] == 7 );
        assert( res2[2] == "ab" );
        assert( res3[2] == "ab ab cd cd" );
        assert( res3[3] == 2 );
    }
    ...
    // errors
    {
        ...
        assert( res1[1].isNil() );
        assert( res1[1].isError() );
        assert( res1[1] == "" );
        assert( res1[1] == 0 );
        assert( res1[1] == false );
        // if error, any index is nil and error
        assert( res1[9].isNil() );
        assert( res1[9].isError() );
    }
}
...
void testLuaFiles()
{
    ...
    // test lua file not found
    {
        ...
        printExpectedError(res);
        assert( startsWith("Read error: cannot open", res.getErrMsg()) );
    }
    ...
    // test file found but bad syntax
    {
        ...
        printExpectedError(res);
        assert( startsWith("Read error: test", res.getErrMsg()) );
    }
    ...
    // test file found but semantc error
    {
        ...
        printExpectedError(res);
        assert( startsWith("Logic error in: test", res.getErrMsg()) );
    }
...
void testClassObjRef()
{
    ...
    // using bound method: 
    {
        ...
        LuaBoundMethRef meth2( res[1], res[2] );
...
void testTableRef()
{
    ...
    // metatables
    {
        ...
        assert( ref1.getMetaTable()[1].isNil() );
...
void testObject()
{
    ...
    LuaMethRef m2 ( lua.eval("obj, obj.meth") ); // use LuaTempResult [1] and [1] for obj and methName
